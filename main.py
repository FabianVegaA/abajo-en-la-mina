...
enemies = [FlyingEnemy(enemy[0], enemy[1], "Enemigo") for enemy in enemys]
enemy_bullets = []
...
while not menu:
    ...
    if player.player_movement[0] > 0 or player.player_movement[0] < 0:
        if boot_cooldown_sfx <= 0:
            sound = random.choice(
                ["assets/sound_effects/boot1.wav", "assets/sound_effects/boot2.wav"]
            )
    ...
    tile_rects = framework.render_tiles(
        display,
        scroll,
        tiles,
        [player.player_rect.x - scroll[0], player.player_rect.y - scroll[1]],
        block_dict,
    )
    ...
    if not dead:
        ...
        if not has_spawned_death_particles:
            for x in range(50):
                particles.append(
                    [
                        player.x + random.randrange(-3, 3) - scroll[0],
                        player.y + random.randrange(0, 10) - scroll[1],
                        random.randrange(-3, 3),
                        10,
                        3,
                        random.choice([(232, 123, 67), (245, 161, 93)]),
                    ]
                )
    ...
    for event in pygame.event.get():
        ...
        if (
            ...
            rel_x, rel_y = (
                mp[0] - player.player_rect.x - scroll[0],
                mp[1] - player.player_rect.y - scroll[1],
            )
    ...
    for bullet in bullets:
        ...
        for enemy in enemies:
            ...
            if pygame.Rect(
                bullet.x - scroll[0] - 25, bullet.y - scroll[1] - 25, 16, 16
            ).colliderect(
                pygame.Rect(
                    enemy.x - scroll[0] + enemy.rect.width,
                    enemy.y - scroll[1] + enemy.rect.height,
                    enemy.rect.width,
                    enemy.rect.height,
                )
            ):
                ...
                for x in range(50):
                    ...
                    particles.append(
                        [
                            bullet.x + random.randrange(-20, 20),
                            bullet.y + random.randrange(-10, 10),
                            random.randrange(-3, 3),
                            10,
                            3,
                            random.choice([(221, 66, 70), (223, 214, 138)]),
                        ]
                    )
        ...
        for tile in tile_rects:
            if pygame.Rect(
                tile[0] - scroll[0], tile[1] - scroll[1], tile[2], tile[3]
            ).colliderect(
                pygame.Rect(bullet.x - scroll[0], bullet.y - scroll[1], 4, 4)
            ):
                if pygame.Rect(
                    bullet.x - scroll[0] - 25, bullet.y - scroll[1] - 25, 70, 70
                ).colliderect(
                    pygame.Rect(
                ...
                for enemy in enemies:
                    if pygame.Rect(
                        bullet.x - scroll[0] - 25, bullet.y - scroll[1] - 25, 50, 50
                    ).colliderect(
                        pygame.Rect(
                            enemy.x - scroll[0] + enemy.rect.width,
                            enemy.y - scroll[1] + enemy.rect.height,
                ...
                circles.append(
                    [
                        bullet.x + random.randrange(-10, 10),
                        bullet.y + random.randrange(-10, 10),
                        10,
                        10,
                    ]
    ...
    for particle in particles:
        ...
        pygame.draw.circle(
            display,
            particle[5],
            (particle[0] - scroll[0], particle[1] - scroll[1]),
            particle[4],
        )
    ...
    for circle in circles:
        ...
        pygame.draw.circle(
            display,
            random.choice([(221, 66, 70), (223, 214, 138)]),
            (circle[0] - scroll[0], circle[1] - scroll[1]),
            circle[2],
            circle[3],
        )
    ...
    if int(gold_count) >= gold_per_level[map_index]:
        ...
        if map_index != 3:
            pygame.draw.circle(
                display, (0, 0, 0), (300 // 2, 233.33 // 2), circle_radius
            )
    ...
    for i in reversed(range(1)):
        pygame.draw.circle(
            ...
            (0, 0, 0, 0 + i * 50),
            (
                player.player_rect.x - scroll[0] + 3,
                player.player_rect.y - scroll[1] + 4,
            ),
            (40 + i * 5),
        )
    ...
    for light in lights:
        ...
        if flame_cooldown <= 0:
            framework.entities.append(
                Entity(
                    random.randrange(light[0], light[0] + 10),
                    light[1] + 5,
                    None,
                    255,
                    framework.flame_effect,
                    [0, -0.3],
                    random.choice([(221, 66, 70), (223, 214, 138)]),
                )
            )
    ...
    for g in gold:
        if pygame.Rect(
            ...
            player.player_rect.width,
            player.player_rect.height,
        ).colliderect(g[0] - scroll[0], g[1] - scroll[1], 16, 16):
            framework.play_sound("assets/sound_effects/pickup.wav")
            gold.remove(g)
            for x in range(30):
                particles.append(
                    [
                        g[0] + random.randrange(-20, 20),
                        g[1] + random.randrange(-10, 10),
                        random.randrange(-3, 3),
                        3,
                        2,
                        (255, 184, 74),
        ...
        if enemy.name == "FlyingEnemy":
            ...
            if enemy.bullet_cooldown <= 0 and enemy.dist < 150:
                for x in range(len(projectiles)):
                    enemy_bullets.append(
                        [
                            enemy.x + skeleton_imgs[0].get_width() / 2,
                            enemy.y + skeleton_imgs[0].get_height() / 2,
                            projectiles[x],
                            300,
                        ]
                    )
            ...
            for tile in fall_tiles:
                ...
                pygame.draw.line(
                    ...
                    (212, 30, 60),
                    (
                        enemy.x - scroll[0] + skeleton_imgs[0].get_width() / 2,
                        enemy.y - scroll[1] + skeleton_imgs[0].get_height() / 2,
                    ),
                    (tile[0] - scroll[0] + 8, tile[1] - scroll[1] + 8),
                    enemy.width,
                )
    ...
    for bullet in enemy_bullets:
        pygame.draw.circle(
            shadows,
            (212, 30, 60, 100),
            (bullet[0] - scroll[0], bullet[1] - scroll[1]),
            2 + abs(np.sin(start_time) * 5),
        )
        bullet[0] += bullet[2][0]
        bullet[1] += bullet[2][1]
        bullet[3] -= 1
        pygame.draw.circle(
            display, (212, 30, 60), (bullet[0] - scroll[0], bullet[1] - scroll[1]), 2
        )

        if (
            pygame.Rect(
                bullet[0] - scroll[0] - 2, bullet[1] - scroll[1] - 2, 4, 4
            ).colliderect(
                player.player_rect.x - scroll[0],
                player.player_rect.y - scroll[1],
                player.player_rect.width,
    ...
    gold_count_text = framework.render_fps_font(
        FPS_FONT, f"Oro: {gold_count}/{gold_per_level[map_index]}"
    )
