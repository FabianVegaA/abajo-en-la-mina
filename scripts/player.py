...
class Player:
    ...
    def get_input(self, dt):
        ...
        if not keys[pygame.K_d] and not keys[pygame.K_a]:
            self.current_animation = 0 if self.air_timer < 6 else 2
        ...
        self.vertical_momentum = min(self.vertical_momentum, 3)

    def check_collisions(self, tile_rects, scroll):
        return [
            pygame.Rect(tile[0], tile[1], 16, 16)
            ...
            if pygame.Rect(
                ...
                pygame.Rect(tile[0] - scroll[0], tile[1] - scroll[1], tile[2], tile[3])
            )
        ]
    ...
    def draw(self, display, scroll):
        self.animation_index = animate(
            self.animations[self.current_animation], self.animation_index, 10
        )
        image = self.animations[self.current_animation][self.animation_index // 10]

        display.blit(
            pygame.transform.flip(
                pygame.transform.rotate(image, self.rotation), self.flipped, False
            ),
            (self.player_rect.x - scroll[0], self.player_rect.y - scroll[1] - 2),
        )

        mouse_x, mouse_y = pygame.mouse.get_pos()
        rel_x, rel_y = (
            mouse_x - self.player_rect.x - scroll[0] + image.get_width() / 2,
            mouse_y - self.player_rect.y - scroll[1],
        )
